...
def _pyserini_hit_raw_json(hit):
    """JSON string stored in the Lucene index `raw` field. Pyserini hit API differs by version."""
    raw = getattr(hit, "raw", None)
    if raw:
        return str(raw)
    lucene_doc = getattr(hit, "lucene_document", None)
    if lucene_doc is not None:
        r = lucene_doc.get("raw")
        if r is not None:
            return str(r)
    raise AttributeError(
        f"Cannot read indexed document JSON from hit ({type(hit)}); "
        "upgrade/downgrade pyserini or rebuild indexes per README."
    )
...
def retrieve_relevant_hits(searcher, queries):
    ...
    for query, q_id in zip(queries, q_ids):
        ...
        hits = list(dict.fromkeys([_pyserini_hit_raw_json(hit) for hit in hits]))
...
def obtain_n_grams(sequence, max_n):
    """
    N-grams for BM25 query variants. Uses NLTK when available; otherwise whitespace tokens.
    (NLTK/pyserini are only needed when --db_content_index_path is set.)
    """
    try:
        ...
        for n in range(1, max_n + 1):
            all_n_grams.extend([" ".join(gram) for gram in nltk_ngrams(tokens, n)])
        return all_n_grams
    except ImportError:
        ...
        for n in range(1, max_n + 1):
            ...
            for i in range(len(tokens) - n + 1):
                all_n_grams.append(" ".join(tokens[i : i + n]))
        return all_n_grams
